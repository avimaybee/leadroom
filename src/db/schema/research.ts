...
export const jobRuns = sqliteTable('job_runs', {
  ...
  status: text('status').notNull().$type<'QUEUED' | 'RUNNING' | 'COMPLETED' | 'FAILED' | 'CANCELLED'>(),
  ...
  jobMeta: text('job_meta', { mode: 'json' }).$type<string>(),
...
export const researchSnapshots = sqliteTable('research_snapshots', {
  ...
  sources: text('sources', { mode: 'json' }), // JSON array of string URLs
