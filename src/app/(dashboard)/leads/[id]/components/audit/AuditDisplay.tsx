...
export function AuditDisplay({
  ...
  // Parse scoring factors from database JSON string
  const factorsList = score?.factors
    ? JSON.parse(score.factors) as Array<{ name: string; value: number; description: string }>
    : [];
  ...
  return (
    <div className="space-y-8">
      {/* Top Header & Priority Score Card */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        
        {/* Core Priority Gauge Card */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Lead Priority Score</span>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-4xl font-black text-slate-900">{score?.scoreValue ?? 0}</span>
              <span className="text-sm font-semibold text-slate-400">/100</span>
            </div>
            {score && (
              <span className={`inline-flex items-center px-2.5 py-0.5 mt-2 rounded-lg text-xs font-bold ${getPriorityLabelColor(score.scoreLabel)}`}>
        ...
        {/* Override / Run Audit Card */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between md:col-span-2 space-y-4">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Operational Controls</span>
              <h4 className="text-sm font-bold text-slate-900 mt-1">Audit & Manual Override Settings</h4>
            </div>
            <div className="flex gap-2">
              ...
              <button
                onClick={onRunAudit}
                disabled={isAuditing}
                className="bg-slate-900 hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition flex items-center gap-2"
              >
                {isAuditing ? (
                  <>
            ...
            <form action={formAction} className="border-t border-slate-100 pt-4 space-y-4 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">New Score (0-100)</label>
                  <input
                    type="number"
                    name="scoreValue"
                    required
                ...
                <div className="sm:col-span-3 space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Override Rationale / Justification</label>
                  <input
                    type="text"
                    name="rationale"
                    required
      ...
      {/* Audit Breakdown and Scores Grid */}
      {audit ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          
          {/* Left Sub-scores progress cards */}
          <div className="lg:col-span-1 space-y-4">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Digital Sub-categories</h4>
          ...
          {/* Right Audit recommendations and observations */}
          <div className="lg:col-span-2 space-y-6">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Observations & Opportunities</h4>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest block">Key Strengths</span>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
                <span className="text-[10px] font-bold text-rose-600 uppercase tracking-widest block">Key Weaknesses</span>
                <div className="text-xs text-slate-800 font-medium leading-relaxed bg-slate-50/30 p-3 rounded-xl border border-slate-200/30 prose-markdown">
                  {audit.keyWeaknesses ? (
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-3">
              <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest block">Recommended Creative Improvements</span>
            ...
            {factorsList.length > 0 && (
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200/60 space-y-4">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Priority Score Driver Breakdown</h4>
                <div className="space-y-3">
                  {factorsList.map((f, idx) => (
                    <div key={idx} className="flex justify-between items-start text-xs border-b border-slate-200/30 pb-2.5 last:border-0 last:pb-0">
                      <div>
                        <span className="font-bold text-slate-700 block">{f.name}</span>
                        <span className="text-[10px] text-slate-400 font-medium">{f.description}</span>
                      </div>
                      <span className={`font-black shrink-0 ${f.value > 0 ? 'text-emerald-600' : (f.value < 0 ? 'text-rose-600' : 'text-slate-500')}`}>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-4">
          ...
          <button
            onClick={onRunAudit}
            disabled={isAuditing}
            className="bg-slate-900 hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition"
          >
